...
- **Engine**: `tools/prototype-studio/` — the vendored `salesforce-ux/design-system-2-starter-kit` (LWC + Vite + SLDS2 + Lightning Base Components, synthetic shadow DOM). See `tools/README.md`. One shared local instance for the whole project — you add a page to it per capability, you don't fork a new instance per capability.
- **Domain → app mapping**: each domain (`docs/sdd/DOMAINS.md`) is an "app" in `tools/prototype-studio/src/apps.config.js` (create the domain's app entry the first time any of its capabilities gets a prototype, reuse it after). Each capability's screen(s) become page component(s) registered under that app's `pages` list — this makes the domain boundary visible in the prototype itself, not just in our docs.
- **Output copied into the spec**: `specs/<domain>/<NNN>-<slug>/prototype/` holds a copy of the capability's page/component source files (`.js`, `.html`, `.css`) plus a `prototype/README.md` with the exact steps to drop them into `tools/prototype-studio/` and run it. This is what satisfies constitution Principle IX (a fresh agent can build/verify from the folder alone) — `tools/prototype-studio/` is the shared engine, the capability's `prototype/` folder is the portable proof.
...
- **Modals**: extend `lightning/modal`, following `tools/prototype-studio/src/modules/ui/demoModal/` as the reference — never hand-build from raw `slds-modal` markup.
