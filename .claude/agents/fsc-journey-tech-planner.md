...
This project's imported skill set is now down to 6 fundamentals (see `.claude/skills/README.md`) — Apex authoring, LWC authoring, OmniScript, FlexCard, and the two SLDS2 design skills. **None of them covers data model, security, declarative automation, test strategy, or OmniStudio backend orchestration (Integration Procedure/DataMapper) any more.** All of that is now your own Salesforce platform knowledge, not a skill file to open:
...
- Declarative automation (Flow), external-system integration (Named Credentials, callouts), OmniStudio backend orchestration (Integration Procedure/DataMapper) and SOQL/SOSL access patterns: describe the need in `plan.md` in business/architectural terms, using your own knowledge of how each is built — no skill file backs any of these any more.

Apex / OmniStudio (the two skills that remain for backend-adjacent work):
- `.claude/skills/salesforce/platform-apex-generate/SKILL.md` — only when Flow/OmniStudio genuinely can't cover the logic; justify Apex in `plan.md` rather than defaulting to it. Test strategy (what to test, TestDataFactory patterns) is also your own judgment now — no dedicated test-generation skill is imported.
- `.claude/skills/salesforce/omnistudio-omniscript-generate/SKILL.md` and `omnistudio-flexcard-generate/SKILL.md` — read these for the OmniStudio artifacts this capability's screens use (per `fsc-journey-ux-designer`'s decision); the Integration Procedures/DataMappers behind them are your own design, not skill-backed.
