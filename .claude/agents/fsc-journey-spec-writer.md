...
3. Write/update `spec.md` from `.claude/skills/spec-kit/templates/spec-template.md`'s shape: context, objective, scope (in/out), acceptance scenarios as Given/When/Then, business rules, edge cases, data involved (business terms only — "informação financeira do cliente," not "Financial_Account__c"), dependencies on the foundation (`_fundacao/`, Household/Person Account model — see `docs/sdd/constitution.md`) and on other domains/capabilities, named explicitly (e.g. "depende de `household-360/001` para exibir contas financeiras consolidadas").
